...
class LSTM(RecurrentLayer):
    ...
    def fprop(self, XH, tparams = None):
        ...
        for h, (recname, recout) in izip(H, self.recurrent.items()):
            U = self.params['U_'+recname+'__'+self.name]
            z += T.dot(h[:, :recout].astype('float32'), U.astype('float32'))
...
class GFLSTM(LSTM):
    ...
    def initialize(self):
        ...
        params['b_'+self.name] = self.init_b.get(4*N+Nm, 'b_' + self.name)
...
class GRU(RecurrentLayer):
    ...
    def initialize(self):
        ...
        params['b_'+self.name] = self.init_b.get(3*N, 'b_' + self.name)
...
class GFGRU(GRU):
    ...
    def initialize(self):
        ...
        params['b_'+self.name] = self.init_b.get(3*N+Nm, 'b_' +self.name) 
